...
Say they see that you have folders organized by year and month, each containing dozens of files between 2-8 megabytes,
then they can say with fair certainty that you are storing a photo library there—and potentially identify which events or trips you documented based on the number of photos and dates.
Or say you store a copy of leaked confidential documents (not that we recommend that),
authorities could ask your cloud provider to hand over your encrypted files and analyze them.
When the number of files, file sizes and directory structure match the leaked documents, they know you're storing them.
