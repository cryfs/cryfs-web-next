...
- **Pirated movies or TV shows**: Large files of 1-4 gigabytes with sizes matching known releases make identification trivial, since encoding groups use standard settings.
...
- **Medical imaging**: X-rays, MRIs, and CT scans stored as DICOM files have characteristic sizes that could expose sensitive health information.
- **Leaked confidential documents**: If authorities request your encrypted files from your cloud provider, they can match the file structure against known leaks to identify what you're storing.
